...
class State:
    ...
    def get_slot(self,i,j):
        return self.__board[i*ROWS+j] if i*ROWS+j < ROWS*COLS else None
    ...
    def move_state(self, col: int):
        ...
        newBoard = self.__board[ : row*ROWS+col] + self.__nextTurn + self.__board[row*ROWS+col+1 : ]
    ...
    def get_heuristic(self):
        h = 0
        for i in range(4):
            h += (i+1) * self.__chains1[i]
            h -= (i+1) * self.__chains2[i]
        return h
    ...
    def __update_chains(self, x: int, y: int, pieceP: str):
        ...
        chainsP, chainsO = self.__chains1, self.__chains2 if pieceP == '1' else self.__chains2, self.__chains1
        ...
        for (offsetRow,offsetCol) in offsets:
            ...
            if slot == pieceP: 
                ...
                if (chainLenF in range(1,5)): chainsP[chainLenF-1] -= 1 # 1 <= length <= 4
                else: chainsP[3] -= 4*(chainLenF-3) # length > 4
            ...
            if newChainLen < 4 and (boundaryF == pieceO or boundaryF is None) and (boundaryB == pieceO or boundaryB is None): continue
            # Add the new chain length to the chains otherwise
            if (newChainLen in range(1,5)): chainsP[newChainLen-1] += 1
            elif (newChainLen >= 5): chainsP[3] += 4*(newChainLen-3)
